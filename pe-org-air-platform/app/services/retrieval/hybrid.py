...
class HybridRetriever:
    ...
    def __init__(
        ...
        # Load BM25 from existing Chroma data on startup
        self._load_bm25_from_store()

    def _load_bm25_from_store(self):
        """Seed BM25 from existing vector store documents."""
        # BM25 starts empty — gets populated as documents are indexed
        # Full rebuild would require fetching all docs from Chroma Cloud
        # which is expensive. BM25 is rebuilt incrementally via index_documents()
        pass

    def refresh_sparse_index(self):
        """Rebuild BM25 — call after indexing new documents."""
        if self._doc_store and _BM25_AVAILABLE:
            self._tokenized_corpus = [d.content.lower().split() for d in self._doc_store]
            self._bm25 = BM25Okapi(self._tokenized_corpus)
    ...
    def index_documents(self, documents: List[RetrievedDocument]) -> int:
        ...
        # Convert RetrievedDocument to a format VectorStore can handle
    ...
    def _dense_search(
        ...
        # Build filter kwargs for VectorStore.search()
