...
# # # CS4 spec requires all-MiniLM-L6-v2 (384-dim, cosine similarity)
# # # Changed from BAAI/bge-small-en-v1.5 to match spec exactly
# # EMBEDDING_MODEL = "all-MiniLM-L6-v2"
# # # EMBEDDING_MODEL = 'BAAI/bge-small-en-v1.5'  # TODO: switch to this if we want better quality and don't mind 384→768 dim change
...
#                 "page_number": str(ev.page_number or ""),
#             }
#             content_truncated = ev.content[:2000]
#             documents.append(content_truncated)
#             _hash = hashlib.sha256(content_truncated.encode()).hexdigest()[:16]
#             ids.append(ev.evidence_id or f"ev_{_hash}")
#             metadatas.append(meta)
...
#             if total == 0:
#                 return []
#             results = self._cloud_query(query_emb, min(top_k, total), where)
#             if not results:
#                 return []
#             output = []
