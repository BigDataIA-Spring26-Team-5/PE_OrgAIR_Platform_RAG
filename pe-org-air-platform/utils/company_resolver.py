...
def _ticker_from_sec_edgar(company_name: str) -> Optional[str]:
    """
    Search SEC EDGAR company tickers file for a company name.
    More reliable than Groq for well-known public companies.
    """
    try:
        resp = requests.get(
            SEC_EDGAR_TICKERS,
        ...
        for _, company in data.items():
            if company.get("title", "").lower() == name_lower:
                return company.get("ticker", "").upper()
        # Try partial match
        for _, company in data.items():
        ...
        logger.warning("sec_name_resolution_failed name=%s error=%s", company_name, e)
    return None
...
def _ticker_from_name_groq(company_name: str) -> Optional[str]:
    ...
    First tries SEC EDGAR (more reliable), then falls back to Groq.
    """
    # Try SEC EDGAR first — more accurate for public companies
    sec_ticker = _ticker_from_sec_edgar(company_name)
    if sec_ticker:
        return sec_ticker
        ...
        resp = requests.post(
            ...
            json={
                ...
                "messages": [{
                    "role": "user",
                    "content": (
                        f"What is the official NYSE or NASDAQ stock ticker symbol for "
                        f"the company named '{company_name}'?\n"
                        f"Rules:\n"
                        f"- Reply with ONLY the ticker symbol\n"
                        f"- Must be 1-5 uppercase letters only\n"
                        f"- No explanations, no punctuation\n"
                        f"- Examples: MSFT, AAPL, GOOGL, AMZN, META\n"
                        f"Ticker:"
                    ),
                }],
                "max_tokens": 10,
